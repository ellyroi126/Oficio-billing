// Alternative migration script that can run with node directly
// Run with: node scripts/migrate-to-r2.js
    ...
    for (const contract of contracts) {
      ...
      if (contract.filePath && !contract.filePath.startsWith('http')) {
        try {
          ...
          console.log(`  ✗ Failed to migrate DOCX: ${error.message}`)
        }
      }
      ...
      if (contract.pdfPath && !contract.pdfPath.startsWith('http')) {
        try {
          ...
          console.log(`  ✗ Failed to migrate PDF: ${error.message}`)
        }
      }
    }
