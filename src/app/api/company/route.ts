...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Include automation fields if provided (with validation)
    ...
    if (body.autoGenerateDay !== undefined) {
      const day = Number(body.autoGenerateDay)
      if (isNaN(day) || day < 1 || day > 28) {
        return NextResponse.json({ success: false, error: 'Auto-generate day must be between 1 and 28' }, { status: 400 })
      }
      companyData.autoGenerateDay = day
    }
    if (body.autoGenerateDaysAhead !== undefined) {
      const days = Number(body.autoGenerateDaysAhead)
      if (isNaN(days) || days < 1 || days > 365) {
        return NextResponse.json({ success: false, error: 'Days ahead must be between 1 and 365' }, { status: 400 })
      }
      companyData.autoGenerateDaysAhead = days
    }
